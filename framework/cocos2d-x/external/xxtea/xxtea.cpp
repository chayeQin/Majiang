...
static const char* jyx_sign = "XXJYX";
static const xxtea_long jyx_sign_len = strlen(jyx_sign);

static const char* jyx_key = "Ksfxxe_Adddjjds";
static const xxtea_long jyx_key_len = strlen(jyx_key);

bool jyx_check_xxtea(const char *chunk)
{
	if (strncmp(chunk, jyx_sign, jyx_sign_len) == 0){
		return true;
	}
...
unsigned char *jyx_xxtea_decrypt(unsigned char *chunk, xxtea_long chunkSize, xxtea_long *ret_length, bool isZip)
{
	unsigned char *result = xxtea_decrypt((unsigned char*)chunk + jyx_sign_len,
										(xxtea_long)chunkSize - jyx_sign_len,
										(unsigned char*)jyx_key,
										(xxtea_long)jyx_key_len,
										ret_length);

	if (isZip)
	{
		unsigned char *tmp;
		unsigned long lenTmp = *ret_length * 10;
		while (true){
			tmp = new unsigned char[lenTmp];
			int r2 = uncompress(tmp, &lenTmp, result, *ret_length);
			if (Z_BUF_ERROR == r2)
			{
				delete[] tmp;
				lenTmp += lenTmp;
			...
			{
				unsigned char* buf = (unsigned char *)malloc(lenTmp);
				memcpy((char*)buf, tmp, lenTmp);
				free(result);
				result = buf;
				*ret_length = lenTmp;
			}
