...
class ScriptsCompiler
{
    ...
    protected function prepareForCompile(array $files)
    {
        ...
        foreach ($files as $key => $path)
        {
            ...
            else
            {
				...
				$found = false;
				foreach ($this->config['excludes'] as $k => $v)
				{
    ...
    protected function compileModules(array $modules, $key = null, $sign = null)
    {
        ...
        foreach ($modules as $path => $module)
        {
			...
			$modulesName = $module['moduleName'];
		
			printf("  > get %s bytes [%d KB] ->", $modulesName,ceil(strlen($bytes) / 1024));
			// Ñ¹Ëõ½Å±¾
			$tempBytes = gzcompress($bytes);
			$d = (int)(strlen($bytes) / strlen($tempBytes));
			if (!empty($key))
			{
				printf("(zip %d)",$d);
				$tempBytes = $sign . $xxtea->encrypt($tempBytes);
			}
			else
			{
				printf("(un key) %s",$key);
			...
			$destPath = $this->config['output'] . DS . 'bin' . DS . str_replace('.', DS, $this->config['prefix'] . $modulesName) . '.' . $this->config['extname'];
			@mkdir(pathinfo($destPath, PATHINFO_DIRNAME), 0777, true);
			file_put_contents($destPath, $tempBytes);
    ...
    {
        // create ZIP archive
		$zipfile = $this->config['output'] . DS . 'game.zip';
		$zip = new ZipArchive();
